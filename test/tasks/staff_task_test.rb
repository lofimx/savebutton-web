...
class StaffTaskTest < ActiveSupport::TestCase
  ...
  test "promote sets role to staff" do
    ...
    capture_io { Rake::Task["staff:promote"].invoke("promote-me@example.com") }
  ...
  test "promote is idempotent for already-staff users" do
    ...
    capture_io { Rake::Task["staff:promote"].invoke("already-staff@example.com") }
    assert_equal "staff", user.reload.role
  end

  test "promote normalizes email" do
    user = create(:user, email_address: "test@example.com")
    capture_io { Rake::Task["staff:promote"].invoke("  TEST@Example.com  ") }
    assert_equal "staff", user.reload.role
  end

  test "promote exits with error for unknown email" do
    error = assert_raises(SystemExit) do
      capture_io { Rake::Task["staff:promote"].invoke("nobody@example.com") }
    end
  ...
  test "promote exits with error when email is missing" do
    error = assert_raises(SystemExit) do
      capture_io { Rake::Task["staff:promote"].invoke }
    end
    assert_equal 1, error.status
  end
  ...
  test "demote sets role back to user" do
    ...
    @rake.tasks.find { |t| t.name == "staff:promote" } # ensure both tasks loaded
    Rake::Task["staff:demote"].reenable
    capture_io { Rake::Task["staff:demote"].invoke("demote-me@example.com") }
    assert_equal "user", user.reload.role
  end

  test "demote exits with error for unknown email" do
    error = assert_raises(SystemExit) do
      capture_io { Rake::Task["staff:demote"].invoke("nobody@example.com") }
    end
    assert_equal 1, error.status
  end
end
